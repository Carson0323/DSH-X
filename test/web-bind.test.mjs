...
test('设置页有 Web 绑定选择框，并会一起提交/回填', () => {
  assert.match(html, /<label class="field"><span data-i18n="Web 绑定">Web 绑定<\/span>\s*<select id="webBind">/, '控件放在高级设置里，复用现有字段样式')
  assert.match(html, /<option value="loopback"[^>]*>回环（127\.0\.0\.1）<\/option>/)
  assert.match(html, /<option value="lan"[^>]*>局域网（0\.0\.0\.0）<\/option>/)
  assert.match(html, /webBind: webBindEl\.value/, '保存时提交')
